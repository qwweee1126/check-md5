...
public class DataBaseOP {
     ...
     * 建立md5table資料表的sql指令
     */
    private static final String C_MD5TABLE = "CREATE TABLE `md5table` (  `fid` int(10) NOT NULL AUTO_INCREMENT,  `md5` varchar(32) NOT NULL,  `hostname` text NOT NULL,  `name` text NOT NULL,  `path` text NOT NULL,  PRIMARY KEY (`fid`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
    /**
     * 建立sametable資料表的sql指令
     */
    private static final String C_SAMETABLE = "CREATE TABLE `sametable` (  `id` int(10) NOT NULL AUTO_INCREMENT,  `md5` varchar(32) NOT NULL,  `hostname` text NOT NULL,  `name` text NOT NULL,  `path` text NOT NULL,  PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
    /**
     ...
     */
    public static boolean insertMD5Table(String md5, String host, String name, String path) {
        boolean havechange = false;
        Connection con = null;
        PreparedStatement pstm = null;
        try {
            con = DatabaseFactory.getInstance().getConnection();
            pstm = con.prepareStatement("INSERT INTO `md5table` (`fid`,`md5`,`hostname`,`name`,`path`) VALUES (NULL,?,?,?,?)");
     ...
     * @return boolean true代表新增成功，false代表新增失敗
     */
    public static boolean insertSameTable(String md5, String host, String name, String path) {
        boolean havechange = false;
        Connection con = null;
        PreparedStatement pstm = null;
        try {
            con = DatabaseFactory.getInstance().getConnection();
            pstm = con.prepareStatement("INSERT INTO `sametable` (`id`,`md5`,`hostname`,`name`,`path`) VALUES (NULL,?,?,?,?)");
            pstm.setString(1, md5);
            pstm.setString(2, host);
            pstm.setString(3, name);
            pstm.setString(4, path);
     ...
     * @return boolean true代表新增成功，false代表新增失敗
     */
    public static boolean insertEmptyTable(String host, String name, String path) {
        boolean havechange = false;
        Connection con = null;
        PreparedStatement pstm = null;
        try {
            con = DatabaseFactory.getInstance().getConnection();
            pstm = con.prepareStatement("INSERT INTO `emptytable` (`id`,`hostname`,`name`,`path`) VALUES (NULL,?,?,?)");
     ...
     */
    public static boolean checkMD5Table(String md5) {
